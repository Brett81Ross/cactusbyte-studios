...
assert "gh release create android-latest dist/*.apk" not in WORKFLOW, "Fantasy release must not republish every Android app"
assert "gh release upload android-latest dist/Fantasy-Football-Matrix.apk --clobber" in WORKFLOW, "Fantasy release must replace only the Fantasy APK asset"

print("Fantasy native splash + permanent signing + isolated release contract: OK")
