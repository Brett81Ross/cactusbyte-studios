...
function userInstallationRef(uid:string,installationId:string){return adminDb().collection("profiles").doc(uid).collection(INSTALLATIONS_SUBCOLLECTION).doc(ownerDocId(installationId))}
...
export async function listOrbitInstallations(uid:string,activeOnly=false):Promise<OrbitLinkedInstallation[]>{
 const snap=await adminDb().collection("profiles").doc(uid).collection(INSTALLATIONS_SUBCOLLECTION).get();
...
export async function issueOrbitRecoveryToken(uid:string,email:string,purpose:OrbitRecoveryPurpose,requestedInstallationId=""){
 ...
 const rateRef=db.collection(RATE_COLLECTION).doc(`${ORBITGATHER_APP_ID}__${uid}`);
 ...
 await db.runTransaction(async tx=>{
  ...
  if(purpose==="restore"){
   const owner=await tx.get(db.collection(OWNER_COLLECTION).doc(ownerDocId(installationId)));
...
export async function confirmOrbitLegacyClaim(token:string,installationId:string,deviceLabel:string){
 ...
 await db.runTransaction(async tx=>{
  ...
  claimedUid=uid;
  const ownerRef=db.collection(OWNER_COLLECTION).doc(ownerDocId(installationId));
  ...
  const bindingRef=db.collection("profiles").doc(uid).collection(INSTALLATIONS_SUBCOLLECTION).doc(ownerDocId(installationId));
...
export async function beginOrbitRestore(token:string){
 ...
 await db.runTransaction(async tx=>{
  ...
  tx.create(db.collection(AUDIT_COLLECTION).doc(),{userId:uid,appId:ORBITGATHER_APP_ID,installationId,action:"restore_begin",operationId,createdAt:FieldValue.serverTimestamp(),createdAtMs:now});
 });
...
export async function finishOrbitRestore(token:string,operationId:string,success:boolean){
 ...
 await db.runTransaction(async tx=>{
  ...
  if(success)tx.set(db.collection("profiles").doc(uid).collection(INSTALLATIONS_SUBCOLLECTION).doc(ownerDocId(installationId)),{updatedAt:FieldValue.serverTimestamp(),updatedAtMs:now,lastSeenAtMs:now,lastRestoredAtMs:now},{merge:true});
  tx.create(db.collection(AUDIT_COLLECTION).doc(),{userId:uid,appId:ORBITGATHER_APP_ID,installationId,action:success?"restore_complete":"restore_failed",operationId,createdAt:FieldValue.serverTimestamp(),createdAtMs:now});
 });
...
export async function revokeOrbitInstallation(uid:string,installationId:string){
 ...
 await db.runTransaction(async tx=>{
  ...
  tx.create(db.collection(AUDIT_COLLECTION).doc(),{userId:uid,appId:ORBITGATHER_APP_ID,installationId,action:"binding_revoked",createdAt:FieldValue.serverTimestamp(),createdAtMs:now});
 });
