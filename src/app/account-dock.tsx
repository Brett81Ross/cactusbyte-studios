...
type MigrationApp={appId:string;name:string;state:"LEGACY"|"BACKUP_READY"|"RESTORE_VERIFIED"|"CUTOVER_READY"|"PERMANENT"|"PLAY_READY";nextGate:string|null;signingIdentityStaged:boolean;backupGate:string;restoreGate:string;cutoverReadinessGate:string;canConsiderCutover:boolean;cutoverAuthorized:boolean;canUninstallLegacy:boolean;instruction:string};
type MigrationCenter={checkedAt:string;signingManifest:{available:boolean;status:string;permanentKeysStaged:boolean};counts:Record<MigrationApp["state"],number>;apps:MigrationApp[];safety:{cutoverAuthorized:boolean;uninstallAuthorized:boolean;googlePlayPublicationAuthorized:boolean;requiresExplicitPerAppOwnerApproval:boolean}};
...
export default function AccountDock(){
 const[hasPro,setHasPro]=useState(false),[owner,setOwner]=useState(false),[open,setOpen]=useState(false),[stats,setStats]=useState<OwnerStats|null>(null),[health,setHealth]=useState<OwnerHealth|null>(null),[migration,setMigration]=useState<MigrationCenter|null>(null),[busy,setBusy]=useState(false),[note,setNote]=useState("");
 ...
 async function loadOwnerHealth(){
  ...
  try{
   ...
   const[statsResponse,healthResponse,migrationResponse]=await Promise.all([
    ...
    fetch("/api/owner/migration",{headers,cache:"no-store",credentials:"include"})
   ]);
   ...
   if(!migrationResponse.ok)throw new Error("Android Migration Center is unavailable.");
   const[nextStats,nextHealth,nextMigration]=await Promise.all([statsResponse.json(),healthResponse.json(),migrationResponse.json()]);
   setStats(nextStats);setHealth(nextHealth);setMigration(nextMigration);setOpen(true);
 ...
 return <aside style={{position:"fixed",right:8,bottom:8,zIndex:90,display:"grid",gap:8,maxWidth:500,width:"calc(100% - 16px)",justifyItems:"end",pointerEvents:"none"}}>
  {open&&owner&&<section style={{pointerEvents:"auto",width:"100%",maxHeight:"min(84dvh,820px)",overflow:"auto",background:"rgba(3,12,11,.985)",border:"1px solid rgba(0,213,190,.28)",borderRadius:16,padding:16,boxShadow:"0 16px 48px rgba(0,0,0,.45)"}}><div style={{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center"}}><div><small style={{opacity:.72,fontSize:13}}>OWNER ONLY · INTERNAL RELEASE EVIDENCE</small><b style={{display:"block",fontSize:18}}>CactusByte Owner Health Center™</b></div><button aria-label="Close owner health" style={{minWidth:48,minHeight:48,fontSize:22}} onClick={()=>setOpen(false)}>×</button></div>
   ...
   {migration&&<div style={{marginTop:14,padding:12,border:"1px solid rgba(240,194,108,.22)",borderRadius:12,background:"rgba(240,194,108,.035)"}}><small style={{opacity:.78,fontSize:13}}>ANDROID MIGRATION CENTER</small><div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:7}}>{(["LEGACY","BACKUP_READY","RESTORE_VERIFIED","CUTOVER_READY","PERMANENT","PLAY_READY"] as MigrationApp["state"][]).map(state=>migration.counts[state]>0&&<span key={state} style={{fontSize:11,padding:"5px 7px",borderRadius:999,border:"1px solid rgba(255,255,255,.1)"}}>{state} · {migration.counts[state]}</span>)}</div><p style={{fontSize:12,opacity:.68,lineHeight:1.45}}>Signing manifest: {migration.signingManifest.status}. Every app remains blocked from uninstall/cutover until backup + restore + readiness evidence are proven and you separately approve that app.</p>{migration.apps.map(app=><div key={app.appId} style={{padding:"9px 0",borderTop:"1px solid rgba(255,255,255,.06)"}}><div style={{display:"flex",justifyContent:"space-between",gap:8,flexWrap:"wrap"}}><b style={{fontSize:13}}>{app.name}</b><span style={{fontSize:11,fontWeight:800,color:app.state==="LEGACY"?"#f0c26c":"#8fffdc"}}>{app.state}</span></div><span style={{display:"block",fontSize:11,opacity:.62,marginTop:3}}>Next gate: {app.nextGate||"Complete"} · signing key {app.signingIdentityStaged?"staged":"unverified"} · backup {app.backupGate.toLowerCase()} · restore {app.restoreGate.toLowerCase()}</span><p style={{fontSize:11,opacity:.68,lineHeight:1.4,margin:"4px 0 0"}}>{app.instruction}</p></div>)}<b style={{display:"block",marginTop:10,fontSize:12,color:"#ffb7b7"}}>UNINSTALL AUTHORIZATION: NO · SIGNING CUTOVER AUTHORIZATION: NO · PLAY PUBLICATION AUTHORIZATION: NO</b></div>}
   ...
   <p style={{fontSize:11,opacity:.5,lineHeight:1.4,margin:"13px 0 0"}}>Last checked {new Date(health.generatedAt).toLocaleString()}. Owner Health and Migration Center are diagnostic/read-only and cannot deploy, uninstall, or authorize Android signing cutover.</p><button style={{width:"100%",minHeight:48,marginTop:12,fontSize:15}} onClick={()=>void loadOwnerHealth()}>Refresh Owner Health</button></>:<p style={{opacity:.72,fontSize:14}}>Loading owner health…</p>}
  </section>}
